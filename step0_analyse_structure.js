...
function extractFusionElements(htmlContent, adv_folder_name, slide_folder_name) {
...
function initFolders() {
  ...
  if (init_flag) {
    console.log(`Folder "ADV__Requests": You should store all the request from client in this folder to easily track.`);
    console.log(`Folder "AA_ADVs": Contains all exported ADVs from Anthill Activator.`);
    console.log(`Folder "AA_csv_structures": After running step 1, the corresponding csv files will be created in each slide of each ADV based on folder AA_ADVs.`);
    console.log(`Folder "EW_csv_structures": After running step 2, the csv structures of Anthill will be converted to the EWizard strcutures. You can ignore the 1st and 2nd steps if you only want to work with EWizard platform.`);
    console.log(`Folder "EW_cloned_ADVs": Using wiz clone <ewizard-adv-url-id>, you have to clone the clonning ADV to get the information after step 3 of slide-id, data-asset-id to avoid manually copying them into the cloned ADVs.`);
    console.log(`Folder "EW_Converted_ADVs": After running step 4, you will have the clean Ewziard slide structures in corresponding ADV folder with structure file for you to copying to the cloned_ADV before using pushing CLI "wiz push" to update the changes to the EWizard platform.`);    
  }
  return true;
}
